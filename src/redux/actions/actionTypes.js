export const SIGNUP_REQUEST = 'SIGNUP_REQUEST';
export const SIGNUP_REQUEST_SUCCESS = 'SIGNUP_REQUEST_SUCCESS';
export const LOGIN_REQUEST = 'LOGIN_REQUEST';
export const LOGIN_REQUEST_SUCCESS = 'LOGIN_REQUEST_SUCCESS';
export const SET_LOGIN_STATE = 'SET_LOGIN_STATE';
export const ADD_PARCEL_REQUEST = 'ADD_PARCEL_REQUEST';
export const ADD_PARCEL_SUCCESS = 'ADD_PARCEL_SUCCESS';
export const GET_ALL_ORDERS_REQUEST = 'GET_ALL_ORDERS_REQUEST';
export const GET_ALL_ORDERS_SUCCESS = 'GET_ALL_ORDERS_SUCCESS';
export const GET_ORDERS_REQUEST = 'GET_ORDERS_REQUEST';
export const GET_ORDERS_SUCCESS = 'GET_ORDERS_SUCCESS';
export const GET_ONE_ORDER = 'GET_ONE_ORDER';
export const GET_ONE_ORDER_SUCCESS = 'GET_ONE_ORDER_SUCCESS';
export const CANCEL_ORDERS_REQUEST = 'CANCEL_ORDERS_REQUEST';
export const CANCEL_ORDERS_SUCCESS = 'CANCEL_ORDERS_SUCCESS';
export const EDIT_ORDERS_DESTINATION_REQUEST = 'EDIT_ORDERS_DESTINATION_REQUEST';
export const EDIT_ORDERS_DESTINATION_SUCCESS = 'EDIT_ORDERS_DESTINATION_SUCCESS';
